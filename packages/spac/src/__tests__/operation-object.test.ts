...
describe("Operation Object (4.8.10)", () => {
  ...
  describe("optional fields", () => {
    it("accepts tags as an array of strings", () => {
      expect(
        validateOperationObject({ ...valid(), tags: ["pets"] }).valid,
      ).toBe(true);
    });

    it("rejects tags as a string", () => {
      expect(validateOperationObject({ ...valid(), tags: "pets" }).valid).toBe(
        false,
      );
    });
    ...
    it("accepts summary as a string", () => {
      expect(
        validateOperationObject({ ...valid(), summary: "List pets" }).valid,
      ).toBe(true);
    });

    it("accepts description as a string", () => {
      expect(
        validateOperationObject({ ...valid(), description: "Returns all pets" })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts operationId as a string", () => {
      expect(
        validateOperationObject({ ...valid(), operationId: "listPets" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts deprecated as a boolean", () => {
      expect(
        validateOperationObject({ ...valid(), deprecated: true }).valid,
      ).toBe(true);
    });

    it("rejects deprecated as a string", () => {
      expect(
        validateOperationObject({ ...valid(), deprecated: "yes" }).valid,
      ).toBe(false);
    });

    it("accepts security as an array of Security Requirement Objects", () => {
      expect(
        validateOperationObject({ ...valid(), security: [{ api_key: [] }] })
          .valid,
      ).toBe(true);
    });

    it("accepts empty security array (removes top-level security)", () => {
      expect(validateOperationObject({ ...valid(), security: [] }).valid).toBe(
        true,
      );
    });
    ...
    it("accepts security with empty object (optional security)", () => {
      expect(
        validateOperationObject({ ...valid(), security: [{}] }).valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateOperationObject({ ...valid(), "x-rate-limit": 100 }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateOperationObject({ ...valid(), rateLimit: 100 }).valid,
      ).toBe(false);
    });
  });
});
