...
describe("Security Requirement Object (4.8.30)", () => {
  ...
  describe("structure", () => {
    ...
    it("accepts single security scheme with empty scopes", () => {
      expect(validateSecurityRequirementObject({ api_key: [] }).valid).toBe(
        true,
      );
    });
  ...
  describe("value type", () => {
    ...
    it("rejects value as a string (must be array)", () => {
      expect(validateSecurityRequirementObject({ api_key: "read" }).valid).toBe(
        false,
      );
    });
    ...
    it("rejects array with non-string elements", () => {
      expect(validateSecurityRequirementObject({ api_key: [42] }).valid).toBe(
        false,
      );
    });

    it("rejects value as a boolean", () => {
      expect(validateSecurityRequirementObject({ api_key: true }).valid).toBe(
        false,
      );
    });
  });
  ...
  describe("no specification extensions", () => {
    it("does not support x- extension fields (values must be string arrays)", () => {
      ...
      expect(
        validateSecurityRequirementObject({ "x-custom": ["scope"] }).valid,
      ).toBe(true);
    });
  });
});
