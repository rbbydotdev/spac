...
describe("Parameter Object (4.8.12)", () => {
  ...
  describe("required fields", () => {
    ...
    it("rejects name as a number", () => {
      expect(validateParameterObject({ ...valid(), name: 42 }).valid).toBe(
        false,
      );
    });
  ...
  describe("schema vs content mutual exclusivity", () => {
    ...
    it("rejects parameter with neither schema nor content", () => {
      expect(validateParameterObject({ name: "id", in: "query" }).valid).toBe(
        false,
      );
    });
  ...
  describe("example vs examples mutual exclusivity", () => {
    it("accepts example", () => {
      expect(
        validateParameterObject({ ...valid(), example: "abc123" }).valid,
      ).toBe(true);
    });
  ...
  describe("style values", () => {
    it('accepts style: "simple" for path parameters', () => {
      expect(
        validateParameterObject({ ...valid(), style: "simple" }).valid,
      ).toBe(true);
    });

    it('accepts style: "label" for path parameters', () => {
      expect(
        validateParameterObject({ ...valid(), style: "label" }).valid,
      ).toBe(true);
    });

    it('accepts style: "matrix" for path parameters', () => {
      expect(
        validateParameterObject({ ...valid(), style: "matrix" }).valid,
      ).toBe(true);
    });
    ...
    it('rejects style: "form" for path parameters', () => {
      expect(validateParameterObject({ ...valid(), style: "form" }).valid).toBe(
        false,
      );
    });
    ...
    it('rejects style: "deepObject" for path parameters', () => {
      expect(
        validateParameterObject({ ...valid(), style: "deepObject" }).valid,
      ).toBe(false);
    });

    it("rejects invalid style value", () => {
      expect(
        validateParameterObject({ ...valid(), style: "invalid" }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("optional common fields", () => {
    it("accepts description", () => {
      expect(
        validateParameterObject({ ...valid(), description: "The pet ID" })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts deprecated as boolean", () => {
      expect(
        validateParameterObject({ ...valid(), deprecated: true }).valid,
      ).toBe(true);
    });
    ...
    it("accepts explode as boolean", () => {
      expect(validateParameterObject({ ...valid(), explode: true }).valid).toBe(
        true,
      );
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateParameterObject({ ...valid(), "x-example": "test" }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateParameterObject({ ...valid(), format: "uuid" }).valid,
      ).toBe(false);
    });
  });
});
