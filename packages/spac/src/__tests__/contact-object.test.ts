...
describe("Contact Object (4.8.3)", () => {
  ...
  describe("optional fields", () => {
    ...
    it("accepts url as a valid URI", () => {
      expect(
        validateContactObject({ url: "https://www.example.com/support" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts email as a valid email address", () => {
      expect(
        validateContactObject({ email: "support@example.com" }).valid,
      ).toBe(true);
    });

    it("rejects email as an invalid email", () => {
      expect(validateContactObject({ email: "not-an-email" }).valid).toBe(
        false,
      );
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(validateContactObject({ "x-slack": "#api-support" }).valid).toBe(
        true,
      );
    });
