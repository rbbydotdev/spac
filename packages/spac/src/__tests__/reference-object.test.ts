...
describe("Reference Object (4.8.23)", () => {
  ...
  describe("$ref format", () => {
    it("accepts local JSON pointer reference", () => {
      expect(
        validateReferenceObject({ $ref: "#/components/schemas/Pet" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts external file with fragment", () => {
      expect(
        validateReferenceObject({ $ref: "definitions.json#/Pet" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts relative path reference", () => {
      expect(
        validateReferenceObject({ $ref: "../schemas/Pet.json" }).valid,
      ).toBe(true);
    });
  });
  ...
  describe("optional fields", () => {
    it("accepts summary (overrides referenced component)", () => {
      expect(
        validateReferenceObject({ ...valid(), summary: "A pet object" }).valid,
      ).toBe(true);
    });
