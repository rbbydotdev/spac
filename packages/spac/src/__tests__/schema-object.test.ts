...
describe("Schema Object (4.8.24)", () => {
  ...
  describe("basic type schemas", () => {
    ...
    it('accepts type: "array"', () => {
      expect(
        validateSchemaObject({ type: "array", items: { type: "string" } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts type as an array (nullable)", () => {
      expect(validateSchemaObject({ type: ["string", "null"] }).valid).toBe(
        true,
      );
    });
  });
  ...
  describe("format", () => {
    it('accepts format: "int32"', () => {
      expect(
        validateSchemaObject({ type: "integer", format: "int32" }).valid,
      ).toBe(true);
    });

    it('accepts format: "int64"', () => {
      expect(
        validateSchemaObject({ type: "integer", format: "int64" }).valid,
      ).toBe(true);
    });

    it('accepts format: "float"', () => {
      expect(
        validateSchemaObject({ type: "number", format: "float" }).valid,
      ).toBe(true);
    });

    it('accepts format: "double"', () => {
      expect(
        validateSchemaObject({ type: "number", format: "double" }).valid,
      ).toBe(true);
    });

    it('accepts format: "password"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "password" }).valid,
      ).toBe(true);
    });

    it('accepts format: "date"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "date" }).valid,
      ).toBe(true);
    });

    it('accepts format: "date-time"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "date-time" }).valid,
      ).toBe(true);
    });

    it('accepts format: "byte"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "byte" }).valid,
      ).toBe(true);
    });

    it('accepts format: "binary"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "binary" }).valid,
      ).toBe(true);
    });

    it('accepts format: "email"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "email" }).valid,
      ).toBe(true);
    });

    it('accepts format: "uri"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "uri" }).valid,
      ).toBe(true);
    });

    it('accepts format: "uuid"', () => {
      expect(
        validateSchemaObject({ type: "string", format: "uuid" }).valid,
      ).toBe(true);
    });

    it("accepts custom/unknown formats (non-validating annotation)", () => {
      expect(
        validateSchemaObject({ type: "string", format: "custom-format" }).valid,
      ).toBe(true);
    });
  });
  ...
  describe("annotations", () => {
    it("accepts title", () => {
      expect(
        validateSchemaObject({ type: "string", title: "Pet Name" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts default", () => {
      expect(
        validateSchemaObject({ type: "string", default: "Fido" }).valid,
      ).toBe(true);
    });

    it("accepts deprecated", () => {
      expect(
        validateSchemaObject({ type: "string", deprecated: true }).valid,
      ).toBe(true);
    });

    it("accepts readOnly", () => {
      expect(
        validateSchemaObject({ type: "string", readOnly: true }).valid,
      ).toBe(true);
    });

    it("accepts writeOnly", () => {
      expect(
        validateSchemaObject({ type: "string", writeOnly: true }).valid,
      ).toBe(true);
    });

    it("accepts examples (JSON Schema keyword)", () => {
      expect(
        validateSchemaObject({ type: "string", examples: ["Fido", "Rex"] })
          .valid,
      ).toBe(true);
    });
  });
  ...
  describe("$ref in schema", () => {
    it("accepts $ref", () => {
      expect(
        validateSchemaObject({ $ref: "#/components/schemas/Pet" }).valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateSchemaObject({ type: "string", "x-go-type": "MyString" }).valid,
      ).toBe(true);
    });
