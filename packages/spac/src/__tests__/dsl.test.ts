...
describe("Api", () => {
  it("creates with name and defaults", () => {
    const api = new Api("3.1", "Test API");
  ...
  it("applies API-level macros", () => {
    const api = new Api("3.1", "Test");
    const addServer = macro.api((a) =>
      a.server({ url: "https://api.test.com" }),
    );
    api.use(addServer);
...
describe("Routes", () => {
  ...
  it("supports all HTTP methods at top level", () => {
    const api = new Api("3.1", "Test");
    api.get("/a").response(Pet);
    api.post("/b").response(Pet);
    api.put("/c").response(Pet);
    api.patch("/d").response(Pet);
    api.delete("/e").response(Pet);
    api.options("/f").response(Pet);
    api.head("/g").response(Pet);
    api.trace("/h").response(Pet);
  ...
  it("chains extension", () => {
    const api = new Api("3.1", "Test");
    api.get("/pets").response(Type.Array(Pet)).extension("rate-limit", 100);
    expect(api._routes[0].extensions["x-rate-limit"]).toBe(100);
  });

  it("extension with x- prefix is not doubled", () => {
    const api = new Api("3.1", "Test");
    api.get("/pets").response(Type.Array(Pet)).extension("x-custom", "val");
    expect(api._routes[0].extensions["x-custom"]).toBe("val");
  });

  it("applies route macros", () => {
    const authed = macro.route((r) =>
      r.security("bearer").error(401, ErrorBody),
    );
    const api = new Api("3.1", "Test");
...
describe("Emit", () => {
  ...
  it("emits tags", () => {
    ...
    const doc = api.emit();
    expect((doc.tags as any[])[0]).toEqual({
      name: "Pets",
      description: "Pet ops",
    });
  });
  ...
  it("emits a simple GET route", () => {
    const api = new Api("3.1", "Test");
    api.get("/pets").response(Type.Array(Pet)).summary("List pets").tag("Pets");
    const doc = api.emit();
  ...
  it("emits per-operation security", () => {
    const api = new Api("3.1", "Test");
    api.get("/pets").response(Type.Array(Pet)).security("bearer");
    const doc = api.emit();
    const op = (doc.paths as any)["/pets"].get;
  ...
  it("generates $ref for named schemas in request body", () => {
    ...
    const doc = api.emit();
    const bodySchema = (doc.paths as any)["/pets"].post.requestBody.content[
      "application/json"
    ].schema;
...
describe("Macros", () => {
  ...
  it("composes multiple route macros", () => {
    ...
    const api = new Api("3.1", "Test");
    api.get("/pets").response(Type.Array(Pet)).use(authed).use(withErrors);
...
describe("Explicit responses map", () => {
  it("emits multiple status codes from responses config", () => {
    const api = new Api("3.1", "Test");
    api.get("/pets").respond(200, Type.Array(Pet)).respond(404, ErrorBody);
    const doc = api.emit();
    const op = (doc.paths as any)["/pets"].get;
    expect(op.responses["200"]).toBeDefined();
    expect(op.responses["404"]).toBeDefined();
  ...
  it("emits ResponseDef with description and contentType", () => {
    ...
    api
      .get("/pets")
      .respond(200, { description: "A list of pets", schema: Type.Array(Pet) })
      .respond(204, noContent("Nothing here"));
    const doc = api.emit();
    const op = (doc.paths as any)["/pets"].get;
