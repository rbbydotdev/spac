...
describe("Security Scheme Object (4.8.27)", () => {
  ...
  describe("type values", () => {
    ...
    it('accepts type: "openIdConnect"', () => {
      expect(validateSecuritySchemeObject(validOpenIdConnect()).valid).toBe(
        true,
      );
    });
  ...
  describe("apiKey type requirements", () => {
    it("requires name for apiKey", () => {
      expect(
        validateSecuritySchemeObject({ type: "apiKey", in: "header" }).valid,
      ).toBe(false);
    });

    it("requires in for apiKey", () => {
      expect(
        validateSecuritySchemeObject({ type: "apiKey", name: "api_key" }).valid,
      ).toBe(false);
    });
  ...
  describe("http type requirements", () => {
    ...
    it('accepts scheme: "bearer"', () => {
      expect(
        validateSecuritySchemeObject({ type: "http", scheme: "bearer" }).valid,
      ).toBe(true);
    });

    it('accepts scheme: "basic"', () => {
      expect(
        validateSecuritySchemeObject({ type: "http", scheme: "basic" }).valid,
      ).toBe(true);
    });
  ...
  describe("oauth2 type requirements", () => {
    it("requires flows for oauth2", () => {
      expect(validateSecuritySchemeObject({ type: "oauth2" }).valid).toBe(
        false,
      );
    });
  ...
  describe("openIdConnect type requirements", () => {
    it("requires openIdConnectUrl", () => {
      expect(
        validateSecuritySchemeObject({ type: "openIdConnect" }).valid,
      ).toBe(false);
    });
  ...
  describe("specification extensions", () => {
    ...
    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateSecuritySchemeObject({ ...validApiKey(), provider: "internal" })
          .valid,
      ).toBe(false);
    });
  });
