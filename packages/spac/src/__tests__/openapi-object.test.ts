...
describe("OpenAPI Object (4.8.1)", () => {
  ...
  describe("field types", () => {
    it("rejects openapi as a number", () => {
      expect(validateOpenApiObject({ ...valid(), openapi: 3.1 }).valid).toBe(
        false,
      );
    });
    ...
    it("rejects info as a string", () => {
      expect(
        validateOpenApiObject({ ...valid(), info: "not an object" }).valid,
      ).toBe(false);
    });
  ...
  describe("openapi field", () => {
    it('accepts "3.1.2"', () => {
      expect(
        validateOpenApiObject({ ...valid(), openapi: "3.1.2" }).valid,
      ).toBe(true);
    });

    it('accepts "3.1.0"', () => {
      expect(
        validateOpenApiObject({ ...valid(), openapi: "3.1.0" }).valid,
      ).toBe(true);
    });

    it('accepts "3.1.1"', () => {
      expect(
        validateOpenApiObject({ ...valid(), openapi: "3.1.1" }).valid,
      ).toBe(true);
    });

    it("rejects empty string", () => {
      expect(validateOpenApiObject({ ...valid(), openapi: "" }).valid).toBe(
        false,
      );
    });
  });
