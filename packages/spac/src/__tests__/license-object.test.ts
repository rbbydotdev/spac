...
describe("License Object (4.8.4)", () => {
  ...
  describe("optional fields", () => {
    it("accepts identifier as an SPDX expression", () => {
      expect(
        validateLicenseObject({ ...valid(), identifier: "Apache-2.0" }).valid,
      ).toBe(true);
    });
    ...
    it("rejects url as a non-URI string", () => {
      expect(
        validateLicenseObject({ ...valid(), url: "not a uri" }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("mutual exclusivity", () => {
    ...
    it("accepts identifier without url", () => {
      expect(
        validateLicenseObject({ ...valid(), identifier: "MIT" }).valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateLicenseObject({ ...valid(), "x-license-type": "permissive" })
          .valid,
      ).toBe(true);
    });
    ...
    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateLicenseObject({ ...valid(), type: "permissive" }).valid,
      ).toBe(false);
    });
  });
});
