...
describe("Info Object (4.8.2)", () => {
  ...
  describe("field types", () => {
    ...
    it("rejects version as a number", () => {
      expect(validateInfoObject({ ...valid(), version: 1.0 }).valid).toBe(
        false,
      );
    });
  });
  ...
  describe("optional fields", () => {
    it("accepts summary", () => {
      expect(
        validateInfoObject({ ...valid(), summary: "A pet store" }).valid,
      ).toBe(true);
    });
    ...
    it("rejects termsOfService as a non-URI string", () => {
      expect(
        validateInfoObject({ ...valid(), termsOfService: "not a uri" }).valid,
      ).toBe(false);
    });

    it("accepts contact as a Contact Object", () => {
      expect(
        validateInfoObject({ ...valid(), contact: { name: "Support" } }).valid,
      ).toBe(true);
    });

    it("accepts license as a License Object", () => {
      expect(
        validateInfoObject({ ...valid(), license: { name: "MIT" } }).valid,
      ).toBe(true);
    });
  });
  ...
  describe("specification extensions", () => {
    ...
    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateInfoObject({ ...valid(), customField: "value" }).valid,
      ).toBe(false);
    });
  });
});
