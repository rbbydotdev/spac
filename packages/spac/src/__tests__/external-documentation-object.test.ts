...
describe("External Documentation Object (4.8.11)", () => {
  ...
  describe("required fields", () => {
    ...
    it("rejects url as a number", () => {
      expect(validateExternalDocumentationObject({ url: 42 }).valid).toBe(
        false,
      );
    });
    ...
    it("rejects non-URI url string", () => {
      expect(
        validateExternalDocumentationObject({ url: "not a uri" }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateExternalDocumentationObject({ ...valid(), "x-icon": "book" })
          .valid,
      ).toBe(true);
    });
    ...
    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateExternalDocumentationObject({ ...valid(), icon: "book" }).valid,
      ).toBe(false);
    });
  });
});
