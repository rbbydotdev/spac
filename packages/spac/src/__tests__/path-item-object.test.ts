...
describe("Path Item Object (4.8.9)", () => {
  ...
  describe("optional fields", () => {
    it("accepts $ref", () => {
      expect(
        validatePathItemObject({ $ref: "#/components/pathItems/pets" }).valid,
      ).toBe(true);
    });

    it("accepts summary", () => {
      expect(validatePathItemObject({ summary: "Pet operations" }).valid).toBe(
        true,
      );
    });
    ...
    it("accepts description", () => {
      expect(
        validatePathItemObject({ description: "Operations on pets" }).valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validatePathItemObject({ "x-controller": "PetController" }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validatePathItemObject({ controller: "PetController" }).valid,
      ).toBe(false);
    });
  });
});
