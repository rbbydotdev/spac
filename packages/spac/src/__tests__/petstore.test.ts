...
function buildPetstore() {
  ...
  // Batch 1: Api-level externalDocs
  api.externalDocs({
    url: "https://petstore.example.com/docs",
    description: "Full API documentation",
  });

  // Batch 3: Api-level extension
  api.extension("x-api-id", "petstore-v1");

  // Batch 5: Reusable component
  ...
  api.group("/pets", (g) => {
    g.tag("pets");
    // Batch 3: Group-level extension
    g.extension("x-resource", "pet");
    // Batch 6: Path item metadata
    ...
    g.get("/")
      ...
      // Batch 1: Operation-level externalDocs
      .externalDocs({ url: "https://petstore.example.com/docs/list-pets" });
    ...
    g.get("/:petId")
      ...
      .respond("default", { description: "Unexpected error", schema: Error })
    ...
    g.post("/")
      .body(CreatePet)
      .bodyDescription("Pet object to be added to the store")
      .bodyRequired(true)
      ...
      .link(201, "GetPetById", {
        ...
        description: "The newly created pet can be fetched using this link",
      })
      // Batch 4: Callback
      .callback("onStatusChange", "{$request.body#/callbackUrl}", (cb) => {
        cb.post("/")
          .body(
            Type.Object({ petId: Type.Integer(), newStatus: Type.String() }),
          )
    ...
    g.head("/:petId")
      .params(Type.Object({ petId: Type.Integer() }))
      .respond(200, noContent("Pet exists"))
      .respond(404, noContent("Pet not found"))
  ...
  api.group("/store", (g) => {
    ...
    g.post("/orders")
      .body(CreateOrder)
      .respond(201, created(Order))
      .summary("Place an order for a pet")
      .operationId("placeOrder")
      .use(authenticated)
      .use(validated)
      .link(201, "GetOrderById", {
        operationId: "getOrder",
        parameters: { orderId: "$response.body#/id" },
      })
      .link(201, "GetPetForOrder", {
        operationId: "getPet",
        parameters: { petId: "$response.body#/petId" },
      });
  ...
  api.group("/users", (g) => {
    ...
    g.post("/")
      .body(CreateUser)
      .respond(201, created(User))
      .summary("Create a new user")
      .operationId("createUser")
      .use(validated)
      .link(201, "GetUserByName", {
    ...
    g.post("/batch")
      ...
      .respond(201, created(Type.Array(User)))
    ...
    g.post("/login")
      ...
      .respond(200, {
        ...
        // Batch 5: Response example
        example: { token: "eyJhbGciOiJIUzI1NiJ9...", expiresIn: 3600 },
      })
      .respond(401, { description: "Invalid credentials", schema: Error })
      .summary("Log in a user")
      .operationId("loginUser")
      .link(200, "GetLoggedInUser", {
        operationId: "getUserByName",
        parameters: { username: "$request.body#/username" },
      });

    g.post("/logout")
      .respond(204, noContent("Logged out"))
  ...
  api
    ...
    .server({
      ...
      description: "Health check only on local",
    });

  api
    .options("/pets")
    .respond(204, noContent("CORS preflight"))
...
describe("Petstore", () => {
  ...
  describe("document structure", () => {
    ...
    it("has info with all fields", () => {
      ...
      expect(spec.info.termsOfService).toBe(
        "https://petstore.example.com/terms",
      );
  ...
  describe("security schemes", () => {
    ...
    it("oauth2 has authorization code flow with scopes", () => {
      ...
      expect(flow.authorizationUrl).toBe(
        "https://petstore.example.com/oauth/authorize",
      );
      expect(flow.tokenUrl).toBe("https://petstore.example.com/oauth/token");
      expect(flow.refreshUrl).toBe(
        "https://petstore.example.com/oauth/refresh",
      );
  ...
  describe("pet operations", () => {
    ...
    it("POST /pets/ — createPet uses authenticated + validated macros", () => {
      ...
      // Link: create → get
      expect(op.responses["201"].links).toEqual({
        GetPetById: {
          operationId: "getPet",
          parameters: { petId: "$response.body#/id" },
          description: "The newly created pet can be fetched using this link",
  ...
  describe("store operations", () => {
    ...
    it("POST /store/orders — placeOrder with 201 and links", () => {
      ...
      // Links: order → get order, order → get pet
      const links = op.responses["201"].links;
      expect(links.GetOrderById).toEqual({
        operationId: "getOrder",
        parameters: { orderId: "$response.body#/id" },
      });
      expect(links.GetPetForOrder).toEqual({
        operationId: "getPet",
        parameters: { petId: "$response.body#/petId" },
      });
    });
  ...
  describe("user operations", () => {
    it("POST /users/ — createUser with validation and link", () => {
      ...
      // Link: create → get user
      expect(op.responses["201"].links.GetUserByName).toEqual({
        operationId: "getUserByName",
        parameters: { username: "$response.body#/username" },
      });
    });
    ...
    it("POST /users/login — returns LoginResponse with headers and link", () => {
      ...
      // Link: login → get user
      expect(ok.links.GetLoggedInUser).toEqual({
        operationId: "getUserByName",
        parameters: { username: "$request.body#/username" },
      });
    });
  ...
  describe("$ref resolution", () => {
    ...
    it("named schemas in responses become $ref", () => {
      const schema =
        spec.paths["/store/inventory"].get.responses["200"].content[
          "application/json"
        ].schema;
    ...
    it("envelope helper wraps $ref in data property", () => {
      const schema =
        spec.paths["/pets/:petId"].get.responses["200"].content[
          "application/json"
        ].schema;
  ...
  // -- Batch 1: externalDocs + jsonSchemaDialect ----------------------------

  describe("externalDocs + jsonSchemaDialect", () => {
    it("emits jsonSchemaDialect on root", () => {
      expect(spec.jsonSchemaDialect).toBe(
        "https://json-schema.org/draft/2020-12/schema",
      );
    });

    it("emits api-level externalDocs", () => {
    ...
    it("emits operation-level externalDocs on listPets", () => {
      expect(spec.paths["/pets"].get.externalDocs).toEqual({
        url: "https://petstore.example.com/docs/list-pets",
      });
    });
  });

  // -- Batch 2: Request body enrichment + cookies + param descriptions ------

  describe("request body enrichment + cookies + param descriptions", () => {
    it("createPet has bodyDescription and bodyRequired", () => {
      const op = spec.paths["/pets"].post;
      expect(op.requestBody.description).toBe(
        "Pet object to be added to the store",
      );
      expect(op.requestBody.required).toBe(true);
    });

    it("getInventory has cookie parameter", () => {
      const op = spec.paths["/store/inventory"].get;
      const cookieParams = op.parameters.filter((p: any) => p.in === "cookie");
      expect(cookieParams).toHaveLength(1);
      expect(cookieParams[0].name).toBe("session_id");
      expect(cookieParams[0].description).toBe("Session identifier");
    });

    it("getPet has petId param with description", () => {
      const op = spec.paths["/pets/:petId"].get;
      const param = op.parameters.find((p: any) => p.name === "petId");
      expect(param.description).toBe("Unique pet identifier");
    });

    it("listPets status param has description", () => {
      const op = spec.paths["/pets"].get;
      const param = op.parameters.find((p: any) => p.name === "status");
      expect(param.description).toBe("Filter by pet status");
    });
  });

  // -- Batch 3: Default response + extensions --------------------------------

  describe("default response + extensions", () => {
    it("getPet has default error response", () => {
      const op = spec.paths["/pets/:petId"].get;
      expect(op.responses.default).toBeDefined();
      expect(op.responses.default.description).toBe("Unexpected error");
    });

    it("api extension x-api-id on root", () => {
      expect(spec["x-api-id"]).toBe("petstore-v1");
    });

    it("group extension x-resource on /pets path item", () => {
      expect(spec.paths["/pets"]["x-resource"]).toBe("pet");
    });
  });

  // -- Batch 4: Webhooks + Callbacks -----------------------------------------

  describe("webhooks + callbacks", () => {
    it("newPetNotification webhook exists", () => {
      expect(spec.webhooks).toBeDefined();
      expect(spec.webhooks.newPetNotification).toBeDefined();
      expect(spec.webhooks.newPetNotification.post.operationId).toBe(
        "onNewPet",
      );
    });

    it("createPet has onStatusChange callback", () => {
      const op = spec.paths["/pets"].post;
      expect(op.callbacks).toBeDefined();
      expect(op.callbacks.onStatusChange).toBeDefined();
      const expr = Object.keys(op.callbacks.onStatusChange)[0];
      expect(expr).toBe("{$request.body#/callbackUrl}");
    });
  });

  // -- Batch 5: Examples + components ----------------------------------------

  describe("examples + reusable components", () => {
    it("login response has example", () => {
      const mt =
        spec.paths["/users/login"].post.responses["200"].content[
          "application/json"
        ];
      expect(mt.example).toEqual({
        token: "eyJhbGciOiJIUzI1NiJ9...",
        expiresIn: 3600,
      });
    });

    it("NotFound reusable response component exists", () => {
      expect(spec.components.responses.NotFound).toEqual({
        description: "The specified resource was not found",
      });
    });

    it("PetExample reusable example component exists", () => {
      expect(spec.components.examples.PetExample).toBeDefined();
      expect(spec.components.examples.PetExample.summary).toBe("A sample pet");
    });
  });

  // -- Batch 6: Header enrichment + path item metadata ----------------------

  describe("header enrichment + path item metadata", () => {
    it("login X-Rate-Limit is enriched header", () => {
      const header =
        spec.paths["/users/login"].post.responses["200"].headers[
          "X-Rate-Limit"
        ];
    ...
    it("/pets path item has summary from group", () => {
  ...
  describe("feature coverage", () => {
    ...
    it("has operations with path, query, and header parameters", () => {
      // path params
      expect(
        spec.paths["/pets/:petId"].get.parameters.some(
          (p: any) => p.in === "path",
        ),
      ).toBe(true);
      // query params
      expect(
        spec.paths["/pets"].get.parameters.some((p: any) => p.in === "query"),
      ).toBe(true);
      // header params
      expect(
        spec.paths["/pets/:petId/image"].post.parameters.some(
          (p: any) => p.in === "header",
        ),
      ).toBe(true);
    });
    ...
    it("uses links for hypermedia navigation", () => {
      // create pet → get pet
      expect(
        spec.paths["/pets"].post.responses["201"].links.GetPetById,
      ).toBeDefined();
      // place order → get order + get pet
      const orderLinks =
        spec.paths["/store/orders"].post.responses["201"].links;
      expect(orderLinks.GetOrderById).toBeDefined();
      expect(orderLinks.GetPetForOrder).toBeDefined();
      // create user → get user
      expect(
        spec.paths["/users"].post.responses["201"].links.GetUserByName,
      ).toBeDefined();
      // login → get user
      expect(
        spec.paths["/users/login"].post.responses["200"].links.GetLoggedInUser,
      ).toBeDefined();
    });

    it("uses response helpers: created, noContent, paginated, envelope, errorSchema", () => {
      // created
      expect(spec.paths["/pets"].post.responses["201"].description).toBe(
        "Created",
      );
      // noContent
      expect(
        spec.paths["/pets/:petId"].delete.responses["204"].description,
      ).toBe("No Content");
      // paginated
      const pag =
        spec.paths["/pets"].get.responses["200"].content["application/json"]
          .schema;
      expect(pag.required).toEqual(
        expect.arrayContaining(["items", "total", "page", "pageSize"]),
      );
      // envelope
      const env =
        spec.paths["/pets/:petId"].get.responses["200"].content[
