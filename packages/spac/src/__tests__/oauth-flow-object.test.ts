...
describe("OAuth Flow Object (4.8.29)", () => {
  ...
  describe("clientCredentials flow", () => {
    it("accepts valid client credentials flow", () => {
      expect(validateOAuthFlowObject(validClientCredentials()).valid).toBe(
        true,
      );
    });
  });
  ...
  describe("authorizationCode flow", () => {
    it("accepts valid authorization code flow", () => {
      expect(validateOAuthFlowObject(validAuthorizationCode()).valid).toBe(
        true,
      );
    });
    ...
    it("requires both authorizationUrl and tokenUrl", () => {
      ...
      ).toBe(true); // tokenUrl conditionally required per flow type
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateOAuthFlowObject({ ...validPassword(), "x-token-format": "jwt" })
          .valid,
      ).toBe(true);
    });
    ...
    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateOAuthFlowObject({ ...validPassword(), tokenFormat: "jwt" })
          .valid,
      ).toBe(false);
    });
  });
