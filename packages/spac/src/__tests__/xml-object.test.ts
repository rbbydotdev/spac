...
describe("XML Object (4.8.26)", () => {
  ...
  describe("optional fields", () => {
    ...
    it("accepts namespace as a non-relative URI", () => {
      expect(
        validateXmlObject({ namespace: "https://example.com/schema/pet" })
          .valid,
      ).toBe(true);
    });

    it("rejects namespace as a relative URI", () => {
      expect(validateXmlObject({ namespace: "../schemas/pet" }).valid).toBe(
        false,
      );
    });
  ...
  describe("full examples", () => {
    ...
    it("accepts XML array wrapping", () => {
      expect(validateXmlObject({ name: "books", wrapped: true }).valid).toBe(
        true,
      );
    });
  });
