...
describe("Tag Object (4.8.22)", () => {
  ...
  describe("optional fields", () => {
    ...
    it("accepts description with CommonMark", () => {
      expect(
        validateTagObject({ ...valid(), description: "**Pets** operations" })
          .valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateTagObject({ ...valid(), "x-display-name": "Pets" }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(validateTagObject({ ...valid(), displayName: "Pets" }).valid).toBe(
        false,
      );
    });
  });
