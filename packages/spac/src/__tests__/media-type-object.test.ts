...
describe("Media Type Object (4.8.14)", () => {
  ...
  describe("optional fields", () => {
    it("accepts schema", () => {
      expect(
        validateMediaTypeObject({ schema: { type: "object" } }).valid,
      ).toBe(true);
    });

    it("accepts example", () => {
      expect(validateMediaTypeObject({ example: { name: "Fido" } }).valid).toBe(
        true,
      );
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(validateMediaTypeObject({ "x-parser": "custom" }).valid).toBe(
        true,
      );
    });
