...
describe("Discriminator Object (4.8.25)", () => {
  ...
  describe("required fields", () => {
    ...
    it("rejects propertyName as a number", () => {
      expect(validateDiscriminatorObject({ propertyName: 42 }).valid).toBe(
        false,
      );
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateDiscriminatorObject({ ...valid(), "x-enum": true }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateDiscriminatorObject({ ...valid(), enumValues: [] }).valid,
      ).toBe(false);
    });
  });
});
