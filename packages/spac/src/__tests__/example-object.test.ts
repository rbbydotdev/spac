...
describe("Example Object (4.8.19)", () => {
  ...
  describe("optional fields", () => {
    it("accepts summary", () => {
      expect(validateExampleObject({ summary: "A cat example" }).valid).toBe(
        true,
      );
    });
    ...
    it("accepts description", () => {
      expect(
        validateExampleObject({ description: "An example of a **cat**" }).valid,
      ).toBe(true);
    });
    ...
    it("accepts value as any type (object)", () => {
      expect(
        validateExampleObject({ value: { name: "Fluffy", tag: "cat" } }).valid,
      ).toBe(true);
    });
    ...
    it("rejects externalValue as a non-URI string", () => {
      expect(validateExampleObject({ externalValue: "not a uri" }).valid).toBe(
        false,
      );
    });
  });
  ...
  describe("value vs externalValue mutual exclusivity", () => {
    ...
    it("accepts value alone", () => {
      expect(validateExampleObject({ value: { name: "Fluffy" } }).valid).toBe(
        true,
      );
    });
    ...
    it("accepts neither value nor externalValue", () => {
      expect(validateExampleObject({ summary: "No example body" }).valid).toBe(
        true,
      );
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(validateExampleObject({ "x-example-id": "cat-1" }).valid).toBe(
        true,
      );
    });
