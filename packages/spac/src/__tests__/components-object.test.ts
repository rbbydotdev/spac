...
describe("Components Object (4.8.7)", () => {
  ...
  describe("optional fields", () => {
    it("accepts schemas map", () => {
      expect(
        validateComponentsObject({ schemas: { Pet: { type: "object" } } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts parameters map", () => {
      expect(
        validateComponentsObject({
          parameters: { PetId: { name: "petId", in: "path" } },
        }).valid,
      ).toBe(true);
    });
    ...
    it("accepts requestBodies map", () => {
      expect(
        validateComponentsObject({
          requestBodies: { NewPet: { content: { "application/json": {} } } },
        }).valid,
      ).toBe(true);
    });
    ...
    it("accepts callbacks map", () => {
      expect(
        validateComponentsObject({ callbacks: { onEvent: {} } }).valid,
      ).toBe(true);
    });

    it("accepts pathItems map", () => {
      expect(
        validateComponentsObject({ pathItems: { petPath: {} } }).valid,
      ).toBe(true);
    });
  });
  ...
  describe("key naming constraints", () => {
    it("accepts alphanumeric keys", () => {
      expect(
        validateComponentsObject({ schemas: { User1: { type: "object" } } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts keys with dots", () => {
      expect(
        validateComponentsObject({
          schemas: { "my.org.User": { type: "object" } },
        }).valid,
      ).toBe(true);
    });
    ...
    it("accepts keys with underscores", () => {
      expect(
        validateComponentsObject({ schemas: { User_Name: { type: "object" } } })
          .valid,
      ).toBe(true);
    });
    ...
    it("rejects keys starting with #", () => {
      expect(
        validateComponentsObject({ schemas: { "#User": { type: "object" } } })
          .valid,
      ).toBe(false);
    });
    ...
    it("rejects empty string key", () => {
      expect(
        validateComponentsObject({ schemas: { "": { type: "object" } } }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("reference objects", () => {
    it("accepts Reference Objects in responses", () => {
      expect(
        validateComponentsObject({
          responses: {
            NotFound: { $ref: "#/components/responses/GenericNotFound" },
          },
        }).valid,
      ).toBe(true);
    });
