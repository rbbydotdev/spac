...
describe("Encoding Object (4.8.15)", () => {
  ...
  describe("optional fields", () => {
    it("accepts contentType as a string", () => {
      expect(
        validateEncodingObject({ contentType: "application/json" }).valid,
      ).toBe(true);
    });
  ...
  describe("constraints", () => {
    ...
    it("rejects allowReserved as a string", () => {
      expect(validateEncodingObject({ allowReserved: "yes" }).valid).toBe(
        false,
      );
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(validateEncodingObject({ "x-encoder": "custom" }).valid).toBe(
        true,
      );
    });
