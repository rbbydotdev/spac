...
describe("Responses Object (4.8.16)", () => {
  ...
  describe("HTTP status codes", () => {
    it("accepts standard status codes", () => {
      expect(
        validateResponsesObject({ "200": { description: "OK" } }).valid,
      ).toBe(true);
    });

    it("accepts 201 status code", () => {
      expect(
        validateResponsesObject({ "201": { description: "Created" } }).valid,
      ).toBe(true);
    });

    it("accepts 404 status code", () => {
      expect(
        validateResponsesObject({ "404": { description: "Not Found" } }).valid,
      ).toBe(true);
    });

    it("accepts 500 status code", () => {
      expect(
        validateResponsesObject({ "500": { description: "Server Error" } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts wildcard status codes (1XX)", () => {
      expect(
        validateResponsesObject({ "1XX": { description: "Informational" } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts wildcard status codes (2XX)", () => {
      expect(
        validateResponsesObject({ "2XX": { description: "Success" } }).valid,
      ).toBe(true);
    });

    it("accepts wildcard status codes (3XX)", () => {
      expect(
        validateResponsesObject({ "3XX": { description: "Redirect" } }).valid,
      ).toBe(true);
    });

    it("accepts wildcard status codes (4XX)", () => {
      expect(
        validateResponsesObject({ "4XX": { description: "Client Error" } })
          .valid,
      ).toBe(true);
    });
    ...
    it("accepts wildcard status codes (5XX)", () => {
      expect(
        validateResponsesObject({ "5XX": { description: "Server Error" } })
          .valid,
      ).toBe(true);
    });
  ...
  describe("default response", () => {
    ...
    it("accepts only default response as the single required response", () => {
      expect(
        validateResponsesObject({ default: { description: "Default" } }).valid,
      ).toBe(true);
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateResponsesObject({ ...valid(), "x-default-error": true }).valid,
      ).toBe(true);
    });
  });
});
