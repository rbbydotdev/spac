...
describe("Server Object (4.8.5)", () => {
  ...
  describe("url field", () => {
    ...
    it("rejects URL with query string", () => {
      expect(
        validateServerObject({ url: "https://api.example.com?key=val" }).valid,
      ).toBe(false);
    });

    it("rejects URL with fragment", () => {
      expect(
        validateServerObject({ url: "https://api.example.com#section" }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("optional fields", () => {
    it("accepts description as a string", () => {
      expect(
        validateServerObject({ ...valid(), description: "Production server" })
          .valid,
      ).toBe(true);
    });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateServerObject({ ...valid(), "x-internal": true }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(validateServerObject({ ...valid(), name: "prod" }).valid).toBe(
        false,
      );
    });
  });
