...
describe("Link Object (4.8.20)", () => {
  ...
  describe("structure", () => {
    ...
    it("accepts link with operationRef", () => {
      expect(
        validateLinkObject({ operationRef: "#/paths/~1pets~1{petId}/get" })
          .valid,
      ).toBe(true);
    });
  });
  ...
  describe("operationRef vs operationId mutual exclusivity", () => {
    ...
    it("accepts operationRef alone", () => {
      expect(
        validateLinkObject({ operationRef: "#/paths/~1pets/get" }).valid,
      ).toBe(true);
    });
  ...
  describe("full example", () => {
    it("accepts a fully populated Link Object", () => {
      const link = {
        ...
        description:
          "The `id` value returned can be used as the `userId` parameter",
        server: { url: "https://api.example.com" },
      };
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateLinkObject({ ...valid(), "x-link-type": "related" }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateLinkObject({ ...valid(), linkType: "related" }).valid,
      ).toBe(false);
    });
  });
});
