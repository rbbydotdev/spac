...
describe("Request Body Object (4.8.13)", () => {
  ...
  describe("optional fields", () => {
    it("accepts description", () => {
      expect(
        validateRequestBodyObject({
          ...valid(),
          description: "A JSON object containing pet info",
        }).valid,
      ).toBe(true);
    });
    ...
    it("accepts required as true", () => {
      expect(
        validateRequestBodyObject({ ...valid(), required: true }).valid,
      ).toBe(true);
    });

    it("accepts required as false", () => {
      expect(
        validateRequestBodyObject({ ...valid(), required: false }).valid,
      ).toBe(true);
    });

    it("rejects required as a string", () => {
      expect(
        validateRequestBodyObject({ ...valid(), required: "yes" }).valid,
      ).toBe(false);
    });
  });
  ...
  describe("specification extensions", () => {
    it("allows x- prefixed extension fields", () => {
      expect(
        validateRequestBodyObject({ ...valid(), "x-body-name": "pet" }).valid,
      ).toBe(true);
    });

    it("rejects unknown fields without x- prefix", () => {
      expect(
        validateRequestBodyObject({ ...valid(), bodyName: "pet" }).valid,
      ).toBe(false);
    });
  });
});
