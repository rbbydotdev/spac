...
export function validateOpenApiObject(data: unknown): ValidationResult {
  ...
  else if (!/^3\.1\.\d+$/.test(data.openapi))
    errors.push("openapi must be a valid 3.1.x version");
  ...
  if ("servers" in data && !isArr(data.servers))
    errors.push("servers must be an array");
  if ("paths" in data && !isObj(data.paths))
    errors.push("paths must be an object");
  if ("webhooks" in data && !isObj(data.webhooks))
    errors.push("webhooks must be an object");
  if ("components" in data && !isObj(data.components))
    errors.push("components must be an object");
  if ("security" in data && !isArr(data.security))
...
export function validateInfoObject(data: unknown): ValidationResult {
  ...
  if ("contact" in data && !isObj(data.contact))
    errors.push("contact must be an object");
  if ("license" in data && !isObj(data.license))
...
export function validateContactObject(data: unknown): ValidationResult {
  ...
  if ("email" in data && !isEmail(data.email))
...
export function validateServerObject(data: unknown): ValidationResult {
  ...
  if (!("url" in data)) errors.push("Missing required field: url");
    ...
    if (data.url.includes("?"))
      errors.push("url must not contain query string");
    if (data.url.includes("#")) errors.push("url must not contain fragment");
  }

  if ("variables" in data && !isObj(data.variables))
...
export function validateServerVariableObject(data: unknown): ValidationResult {
  ...
  if ("enum" in data) {
    if (!isArr(data.enum)) {
      ...
      if (!data.enum.every((v) => isStr(v)))
        errors.push("enum values must be strings");
      if (
        isStr(data.default) &&
        data.enum.length > 0 &&
...
export function validatePathsObject(data: unknown): ValidationResult {
  ...
  for (const key of Object.keys(data)) {
    ...
    if (normalizedPaths.has(normalized)) {
      errors.push(
        `Duplicate templated path: ${key} conflicts with ${normalizedPaths.get(normalized)}`,
      );
    } else {
...
export function validateOperationObject(data: unknown): ValidationResult {
  ...
  if ("deprecated" in data && !isBool(data.deprecated))
...
export function validateExternalDocumentationObject(
  data: unknown,
): ValidationResult {
  if (!isObj(data)) return fail("Must be an object");
  const errors: string[] = [];
...
export function validateParameterObject(data: unknown): ValidationResult {
  ...
  if (isStr(data.in) && data.in === "path") {
    if (data.required !== true)
      errors.push("Path parameters must have required: true");
  }
  ...
  if (!hasSchema && !hasContent)
    errors.push("Must have either schema or content");
...
export function validateRequestBodyObject(data: unknown): ValidationResult {
  ...
  if ("required" in data && !isBool(data.required))
...
export function validateEncodingObject(data: unknown): ValidationResult {
  ...
  if ("style" in data && !isStr(data.style))
    errors.push("style must be a string");
  if ("explode" in data && !isBool(data.explode))
    errors.push("explode must be a boolean");
  if ("allowReserved" in data && !isBool(data.allowReserved))
    errors.push("allowReserved must be a boolean");
...
export function validateResponsesObject(data: unknown): ValidationResult {
  ...
  for (const key of Object.keys(data)) {
    ...
    if (
      key === "default" ||
      STATUS_CODE_RE.test(key) ||
      WILDCARD_RE.test(key)
    ) {
      hasResponse = true;
    } else {
...
export function validateResponseObject(data: unknown): ValidationResult {
  ...
  else if (!isStr(data.description))
...
export function validateHeaderObject(data: unknown): ValidationResult {
  ...
  if ("allowEmptyValue" in data)
    errors.push("allowEmptyValue is not allowed on Header Object");
  if ("allowReserved" in data)
    errors.push("allowReserved is not allowed on Header Object");
...
export function validateDiscriminatorObject(data: unknown): ValidationResult {
  ...
  else if (!isStr(data.propertyName))
  ...
  if ("mapping" in data) {
    if (!isObj(data.mapping)) {
      ...
      for (const [key, value] of Object.entries(data.mapping)) {
        if (!isStr(value))
          errors.push(`mapping value for ${key} must be a string`);
      }
    }
  }
...
export function validateXmlObject(data: unknown): ValidationResult {
  ...
  if ("name" in data && !isStr(data.name)) errors.push("name must be a string");
  if ("prefix" in data && !isStr(data.prefix))
    errors.push("prefix must be a string");
  if ("attribute" in data && !isBool(data.attribute))
    errors.push("attribute must be a boolean");
  if ("wrapped" in data && !isBool(data.wrapped))
    errors.push("wrapped must be a boolean");
...
export function validateSecuritySchemeObject(data: unknown): ValidationResult {
  ...
  if (!SECURITY_SCHEME_TYPES.has(data.type))
    return fail(`Invalid type: ${data.type}`);

  switch (data.type) {
    case "apiKey":
      if (!("name" in data) || !isStr(data.name))
        errors.push("apiKey requires name");
      ...
      break;
    case "http":
      if (!("scheme" in data) || !isStr(data.scheme))
        errors.push("http requires scheme");
      break;
    case "oauth2":
      if (!("flows" in data) || !isObj(data.flows))
        errors.push("oauth2 requires flows");
      break;
    case "openIdConnect":
...
const OAUTH_FLOWS_FIELDS = new Set([
  "implicit",
  "password",
  "clientCredentials",
  "authorizationCode",
...
const OAUTH_FLOW_FIELDS = new Set([
  "authorizationUrl",
  "tokenUrl",
  "refreshUrl",
  "scopes",
...
export function validateSecurityRequirementObject(
  data: unknown,
): ValidationResult {
  if (!isObj(data)) return fail("Must be an object");
  const errors: string[] = [];
  ...
  for (const [key, value] of Object.entries(data)) {
    if (!isArr(value)) {
      ...
      for (const item of value) {
        if (!isStr(item))
          errors.push(`Array values for ${key} must be strings`);
      }
    }
  }
