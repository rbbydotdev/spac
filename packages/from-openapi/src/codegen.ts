...
export function genSharedSchemas(
  ...
  const lines: string[] = [
    ...
    'import { named } from "@spec-spac/spac"',
    "",
  ];
  ...
  for (const key of sorted) {
    ...
    if (!schema) continue;
    const varName =
      refMap.get(`#/components/schemas/${key}`) || schemaVarName(key);
...
export function genEndpointIndex(
  ...
  lines.push('import type { Api } from "@spec-spac/spac"');
  if (sharedImports.size > 0) {
    lines.push(
      `import { ${Array.from(sharedImports).sort().join(", ")} } from "../shared/schemas"`,
    );
  }
  if (localImports.size > 0) {
    lines.push(
      `import { ${Array.from(localImports).sort().join(", ")} } from "./schemas"`,
    );
  }
  lines.push("");
  ...
  // Emit assertVersion with the source spec's openapi version for version policy tracking
  const sourceVersion = opts?.sourceSpecVersion || spec.openapi;
  if (sourceVersion) {
    lines.push(
      `  api.assertVersion(${JSON.stringify(sourceVersion)}, ${JSON.stringify(group.name)})`,
    );
  }
  lines.push("");
...
function genGroupedRoutes(
  ...
  if (prefixParams.length > 0) {
    const paramObj = prefixParams
      .map((p) => `${toSafeKey(p)}: Type.String()`)
      .join(", ");
    lines.push(
      `${ind}api.group(${JSON.stringify(prefix)}, { params: Type.Object({ ${paramObj} }) }, (g) => {`,
    );
  } else {
...
function genSingleRoute(
  ...
  // Responses — split into success and error for chained calls
  const successCodes = ["200", "201", "202", "203", "204"];
  const successEntries = Array.from(op.responses.entries()).filter(([c]) =>
    successCodes.includes(c),
  );
  const errorEntries = Array.from(op.responses.entries()).filter(
    ([c]) => !successCodes.includes(c),
  );

  lines.push(`${ind}${builder}.${op.method}(${JSON.stringify(path)})`);

  // Path params (exclude group-prefix ones)
  const groupPrefixParams = new Set(extractPathParams(groupPrefix));
  const routePathParams = op.pathParams.filter(
    (p) => !groupPrefixParams.has(p.name),
  );
  if (routePathParams.length > 0) {
    const paramProps = routePathParams.map((p) => {
      const schema = p.schema
        ? schemaToTypebox(p.schema, ctx, 2)
        : "Type.String()";
      return `${toSafeKey(p.name)}: ${schema}`;
    });
    lines.push(`${ind}  .params(Type.Object({ ${paramProps.join(", ")} }))`);
  }
  ...
  if (op.headerParams.length > 0) {
    ...
    lines.push(
      `${ind}  .headers(Type.Object({\n${headerProps.join("\n")}\n${ind2}  }))`,
    );
  }

  // Request body
  if (op.requestBody) {
    lines.push(`${ind}  .body(${schemaToTypebox(op.requestBody, ctx, 2)})`);
  }

  // Success responses
  if (successEntries.length === 1 && successEntries[0][0] === "200") {
    lines.push(
      `${ind}  .response(${schemaToTypebox(successEntries[0][1], ctx, 2)})`,
    );
  } else {
    for (const [code, schema] of successEntries) {
      lines.push(
        `${ind}  .respond(${code}, ${schemaToTypebox(schema, ctx, 2)})`,
      );
    }
  }
  ...
  for (const [code, schema] of errorEntries) {
    ...
    lines.push(`${ind}  .error(${key}, ${schemaToTypebox(schema, ctx, 2)})`);
  }

  if (op.summary) lines.push(`${ind}  .summary(${JSON.stringify(op.summary)})`);
  if (op.description)
    lines.push(`${ind}  .description(${JSON.stringify(op.description)})`);
  if (op.operationId)
    lines.push(`${ind}  .operationId(${JSON.stringify(op.operationId)})`);
  ...
  if (op.security) {
    for (const sec of op.security)
      lines.push(`${ind}  .security(${JSON.stringify(sec)})`);
  }
  for (const [key, val] of Object.entries(op.extensions)) {
    lines.push(
      `${ind}  .extension(${JSON.stringify(key)}, ${JSON.stringify(val)})`,
    );
  }

  // Links
  for (const [code, codeLinks] of op.links) {
    for (const [linkName, linkConfig] of Object.entries(codeLinks)) {
      const configStr = JSON.stringify(linkConfig);
      lines.push(
        `${ind}  .link(${code}, ${JSON.stringify(linkName)}, ${configStr})`,
      );
    }
  }
...
export function genRootIndex(
  spec: any,
  groups: OperationGroup[],
  opts?: { specVersion?: string; debug?: boolean },
): string {
  const lines: string[] = [];
  const info = spec.info || {};

  lines.push("import { Api } from '@spec-spac/spac'");
  ...
  // Api constructor
  const specVersion =
    opts?.specVersion || (spec.openapi || "3.1").replace(/\.\d+$/, "");
  const title = info.title || "API";
  const configParts: string[] = [];
  if (opts?.debug) configParts.push(`  debug: true,`);
  if (info.version)
    configParts.push(`  version: ${JSON.stringify(info.version)},`);
  if (info.description)
    configParts.push(`  description: ${JSON.stringify(info.description)},`);
  if (info.license) {
    const licParts: string[] = [`name: ${JSON.stringify(info.license.name)}`];
    if (info.license.url)
      licParts.push(`url: ${JSON.stringify(info.license.url)}`);
    if (info.license.identifier)
      licParts.push(`identifier: ${JSON.stringify(info.license.identifier)}`);
    configParts.push(`  license: { ${licParts.join(", ")} },`);
  }
  if (info.contact) {
    const cParts: string[] = [];
    if (info.contact.name)
      cParts.push(`name: ${JSON.stringify(info.contact.name)}`);
    if (info.contact.email)
      cParts.push(`email: ${JSON.stringify(info.contact.email)}`);
    if (info.contact.url)
      cParts.push(`url: ${JSON.stringify(info.contact.url)}`);
    if (cParts.length > 0)
      configParts.push(`  contact: { ${cParts.join(", ")} },`);
  }
  if (info.termsOfService)
    configParts.push(
      `  termsOfService: ${JSON.stringify(info.termsOfService)},`,
    );

  if (configParts.length > 0) {
    lines.push(
      `export const api = new Api(${JSON.stringify(specVersion)}, ${JSON.stringify(title)}, {`,
    );
  ...
  for (const server of spec.servers || []) {
    if (server.variables && Object.keys(server.variables).length > 0) {
      ...
      if (server.description)
        args.push(`description: ${JSON.stringify(server.description)}`);
  ...
  for (const [name, scheme] of Object.entries(secSchemes)) {
    lines.push(
      `api.securityScheme(${JSON.stringify(name)}, ${JSON.stringify(scheme)})`,
    );
  }
