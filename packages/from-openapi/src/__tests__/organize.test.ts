...
describe("extractPathParams", () => {
  it("extracts params from curly braces", () => {
    expect(extractPathParams("/zones/{zone_id}/dns/{record_id}")).toEqual([
      "zone_id",
      "record_id",
    ]);
  });
...
describe("applyStripPrefixes", () => {
  ...
  it("strips matching prefix", () => {
    expect(
      applyStripPrefixes("/accounts/{account_id}/access/apps", prefixes),
    ).toBe("/access/apps");
  });

  it("returns empty for exact match", () => {
    expect(applyStripPrefixes("/accounts/{account_id}", prefixes)).toBe("");
  });

  it("returns original if no match", () => {
    expect(applyStripPrefixes("/user/tokens", prefixes)).toBe("/user/tokens");
  });

  it("does not strip partial segment match", () => {
    expect(applyStripPrefixes("/accounts/move", prefixes)).toBe(
      "/accounts/move",
    );
  });
});
...
describe("groupByPath", () => {
  ...
  it("groups by first path segment", () => {
    const ops = [makeOp("/pets"), makeOp("/pets/{id}"), makeOp("/users")];
