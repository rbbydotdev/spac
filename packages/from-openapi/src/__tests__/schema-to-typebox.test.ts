...
describe("schemaToTypebox", () => {
  describe("primitives", () => {
    ...
    it("converts string with options", () => {
      expect(
        convert({ type: "string", description: "A name", maxLength: 100 }),
      ).toBe('Type.String({"description":"A name","maxLength":100})');
    });

    it("converts string with format", () => {
      expect(convert({ type: "string", format: "email" })).toBe(
    ...
    it("converts number with constraints", () => {
      expect(convert({ type: "number", minimum: 1, maximum: 100 })).toBe(
    ...
    it("converts boolean with default", () => {
      expect(convert({ type: "boolean", default: false })).toBe(
  ...
  describe("enum", () => {
    it("converts string enum", () => {
      expect(convert({ enum: ["asc", "desc"] })).toBe(
  ...
  describe("object", () => {
    ...
    it("handles empty properties", () => {
      expect(convert({ type: "object", properties: {} })).toBe(
        "Type.Object({})",
      );
    });
  ...
  describe("array", () => {
    it("converts array of strings", () => {
      expect(convert({ type: "array", items: { type: "string" } })).toBe(
