...
export function resolveNonSchemaRefs(value: any, spec: any): any {
  ...
  if (Array.isArray(value))
    return value.map((v) => resolveNonSchemaRefs(v, spec));
...
export function parseOperations(spec: any): ParsedOperation[] {
  ...
  for (const [path, pathItem] of Object.entries(paths)) {
    ...
    const pathLevelParams: any[] = resolveNonSchemaRefs(
    ...
    for (const method of HTTP_METHODS) {
      ...
      // Links from responses
      const links = new Map<string, Record<string, any>>();
      if (resolved.responses) {
        for (const [code, resp] of Object.entries(resolved.responses)) {
          const r = resp as any;
          if (r?.links && typeof r.links === "object") {
            const codeLinks: Record<string, any> = {};
            for (const [linkName, linkObj] of Object.entries(r.links)) {
              const l = resolveNonSchemaRefs(linkObj, spec) as any;
              const link: Record<string, any> = {};
              if (l.operationId !== undefined) link.operationId = l.operationId;
              if (l.operationRef !== undefined)
                link.operationRef = l.operationRef;
              if (l.parameters !== undefined) link.parameters = l.parameters;
              if (l.requestBody !== undefined) link.requestBody = l.requestBody;
              if (l.description !== undefined) link.description = l.description;
              codeLinks[linkName] = link;
            }
            if (Object.keys(codeLinks).length > 0) links.set(code, codeLinks);
          }
        }
      }
