...
Options:
  ...
  --spec-version <ver>     Override OpenAPI spec version (e.g. '3.1'). Defaults to spec's openapi field.
...
const files = await generate({ spec, stripPrefixes, name, specVersion, debug });
const elapsed = performance.now() - t0;

if (!outDir) {
  // Dry-run mode: print stats
  console.log(
    `Dry run — ${files.size} files generated in ${elapsed.toFixed(0)}ms\n`,
  );
  ...
  // File breakdown
  const schemas = Array.from(files.keys()).filter((k) =>
    k.endsWith("schemas.ts"),
  );
  const indexes = Array.from(files.keys()).filter((k) =>
    k.endsWith("index.ts"),
  );
