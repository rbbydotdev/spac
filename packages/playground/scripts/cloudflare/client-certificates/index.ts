...
export function registerClientCertificates(api: Api) {
  api.assertVersion("3.0.3", "ClientCertificates")
  ...
  api.group("/zones/{zone_id}/client_certificates", { params: Type.Object({ zone_id: Type.String() }) }, (g) => {
    g.get("/", {
      ...
      .response(TlsCertificatesAndHostnamesClientCertificateResponseCollection)
      .error("4XX", TlsCertificatesAndHostnamesApiResponseCommonFailure)
    ...
    g.post("/", {
      ...
      .response(TlsCertificatesAndHostnamesClientCertificateResponseSingle)
      .error(
        "4XX",
        Type.Object({
          errors: DlpMessages,
