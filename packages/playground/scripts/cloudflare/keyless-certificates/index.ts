...
export function registerKeylessCertificates(api: Api) {
  api.assertVersion("3.0.3", "KeylessCertificates")

  api.group("/zones/{zone_id}/keyless_certificates", { params: Type.Object({ zone_id: Type.String() }) }, (g) => {
    g.get("/", {})
      .response(TlsCertificatesAndHostnamesKeylessResponseCollection)
      .error(
        "4XX",
        Type.Object({
          errors: DlpMessages,
    ...
    g.post("/", {
      ...
      .response(TlsCertificatesAndHostnamesKeylessResponseSingle)
      .error(
        "4XX",
        Type.Object({
          errors: DlpMessages,
