...
const features: Feature[] = [
  {
    ...
    body: <>Each team owns a TypeScript module. Diffs are line-level — not indentation battles in a 10k-line YAML.</>,
  ...
  },
  {
    ...
    body: (
      <>
        <code className="text-fd-foreground">{`Api<'3.1'>`}</code> in function signatures.{" "}
        <code className="text-fd-foreground">assertVersion()</code> catches team drift before CI even runs.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        Auth, audit headers, <code className="text-fd-foreground">x-</code> extensions — publish once, import
        everywhere.
      </>
    ),
  ...
  return (
    <div className="flex flex-col items-center flex-1 gap-16 px-6 pt-20 pb-24 sm:pt-28">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl w-full text-left">
        {featuresWithHints.map(({ icon: Icon, accent, title, body, hintNode }) => (
          <div
            key={title}
          ...
          >
            {/* Top accent rail */}
            <div aria-hidden className="absolute inset-x-0 top-0 h-px opacity-80" style={{ background: accent }} />
            ...
            <Icon className="size-5 mb-3" style={{ color: accent }} aria-hidden />
            <h3 className="text-sm font-semibold mb-1.5 text-fd-foreground tracking-tight">{title}</h3>
            <p className="text-xs text-fd-muted-foreground leading-relaxed">{body}</p>
      ...
      <div className="max-w-4xl w-full text-left">
        ...
        <div className="shiki-window relative rounded-lg border border-fd-border bg-fd-card overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 px-4 py-2 border-b border-fd-border/60">
            <span className="size-2.5 rounded-full" style={{ background: "var(--feat-dot-red)" }} />
            <span className="size-2.5 rounded-full" style={{ background: "var(--feat-dot-yellow)" }} />
            <span className="size-2.5 rounded-full" style={{ background: "var(--feat-dot-green)" }} />
            <span className="ml-2 text-[10px] text-fd-muted-foreground">index.ts</span>
          </div>
          {/* Shiki-tokenized code block */}
          <div className="shiki-block text-xs leading-relaxed">{compositionNode}</div>
        </div>
        <p className="text-xs text-fd-muted-foreground mt-3 leading-relaxed">
          ...
          <code className="text-fd-foreground">assertVersion()</code>. Drift is caught at compile time, audit time, or
          CI — not after deploy.{" "}
          <Link href="/docs/explanation/why-spac" className="text-fd-primary hover:underline">
            Why spac →
          </Link>
        </p>
      </div>
